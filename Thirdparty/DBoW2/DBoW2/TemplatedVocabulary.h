  ...
  BowVector &v, FeatureVector &fv, int levelsup) const
{
  ...
  // normalize 
  //枚举类型，定义在BowVector.h中
  LNorm norm;
  //GeneralScoring类型的m_scoring_object,  类型声明在ScoringObject.h中
  bool must = m_scoring_object->mustNormalize(norm);
  
  typename vector<TDescriptor>::const_iterator fit;
  //WeightingType类型的m_weighting, 枚举类型
  if(m_weighting == TF || m_weighting == TF_IDF)
  {
    ...
    for(fit = features.begin(); fit < features.end(); ++fit, ++i_feature)
    {
      ...
      if(w > 0) // not stopped
      { 
        //BowVector类型的v, FeatureVector类型的fv
        //id相应叶节点的序号,w该节点权重
        //v的类型为std::map<WordId, WordValue>
        v.addWeight(id, w);
        //nid表示指定层相应节点的序号, i_fecture表示第几个特征点
        //fv的类型为std::map<NodeId, std::vector<unsigned int>>
...
{ 
  ...
  {
    ++current_level;
      //vector<Node> m_nodes, 节点的容器, 即DOW2词典的树
      //vector<NodeId> nodes, 序号的容器
      //nodes表示根节点下子节点的序号
